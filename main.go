/*
Docker Compose Deployment Script for Bamboo

For more information see https://github.com/herzog31/docker-deploy-bamboo-go
*/
package main
...
func main() {
	...
	flag.StringVar(&c.Mode, "mode", "DEPLOY,TEST", "Comma separated list of operations that should be executed. The following operations are available: CLEAR, DEPLOY, TEST, REMOVE.")
	...
	flag.StringVar(&c.ProjectName, "projectName", "", "Project name of Docker composition. Example: production")
	flag.StringVar(&c.ComposeFile, "composeFile", "docker-compose.yml", "Name of Docker Compose file.")
	flag.IntVar(&c.StartTime, "startTime", 10, "Time in seconds the deployment script waits for your containers to start during the TEST operation.")
	flag.StringVar(&c.RemoteWorkingDir, "remoteWorkingDir", "", "Absolute path of remote working directory on the deployment target, where the build artifact is copied and executed.")
	flag.StringVar(&c.LocalWorkingDir, "localWorkingDir", "", "Absolute or relative path to local working directory on the build agent, where the build artifact is stored.")
	...
	flag.BoolVar(&c.ClearVolumes, "clearVolumes", false, "Set true to clear container volumes during CLEAR and REMOVE operations.")
	flag.Parse()
